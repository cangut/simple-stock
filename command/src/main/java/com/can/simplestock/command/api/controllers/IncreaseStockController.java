...
@RestController
@RequestMapping(path = "api/v1/increase-stock")
public class IncreaseStockController {

    private final Logger logger = Logger.getLogger(IncreaseStockController.class.getName());
    private final CommandDispatcher commandDispatcher;

    @Autowired
    public IncreaseStockController(CommandDispatcher commandDispatcher) {
        this.commandDispatcher = commandDispatcher;
    }

    @PutMapping(path = "/{id}")
    public ResponseEntity<BaseResponse> increaseStock(@PathVariable String id, @RequestBody IncreaseStockCommand command) {

        try {
            command.setId(id);
            commandDispatcher.send(command);
            return new ResponseEntity<>(new BaseResponse("Increasing stock is successful"), HttpStatus.OK);
